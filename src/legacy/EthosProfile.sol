...
contract EthosProfile is IEthosProfile, ITargetStatus, AccessControl, UUPSUpgradeable, Common {
  ...
  event AddressClaim(uint256 indexed profileId, address indexed addr, AddressClaimStatus indexed claim);
   ...
   */
  function createProfile(uint256 inviterId) external whenNotPaused onlyNonCompromisedAddress(msg.sender) {
    (bool inviteSenderVerified, bool inviteSenderArchived, bool inviteSenderIsMock) = profileStatusById(inviterId);
   ...
   */
  function inviteAddress(address invitee)
    public
    whenNotPaused
    onlyNonZeroAddress(invitee)
    onlyNonCompromisedAddress(invitee)
  {
    ...
    sentInviteIndexByProfileIdAndAddress[profile][invitee] = profiles[profile].inviteInfo.sent.length;
  ...
  function archiveProfile() external whenNotPaused {
    (bool verified, bool archived, bool mock, uint256 profileId) = profileStatusByAddress(msg.sender);
    if (!verified) {
  ...
  function registerAddress(address addressStr, uint256 profileId, uint256 randValue, bytes calldata signature)
    ...
    onlyNonCompromisedAddress(addressStr)
  {
   ...
   */
  function profileStatusByAddress(address addressStr)
    public
    view
   ...
   */
  function addressBelongsToProfile(address addressStr, uint256 profileId) external view returns (bool) {
   ...
   */
  function profileExistsAndArchivedForId(uint256 profileId) external view returns (bool verified, bool archived) {
   ...
   * @return Hash.
   */
  function _keccakForRegisterAddress(address addressStr, uint256 profileId, uint256 randValue)
    private
    pure
    returns (bytes32)
  {
   ...
   */
  function addInvitesBatch(address[] calldata users, uint256 amount) external onlyAdmin whenNotPaused {
