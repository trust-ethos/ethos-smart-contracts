...
contract InteractionControl is Ownable {
  ...
  function removeControlledContractName(string calldata contractName) external onlyOwner {
    for (uint256 i = 0; i < controlledContractNames.length; ++i) {
      if (keccak256(abi.encodePacked(controlledContractNames[i])) == keccak256(abi.encodePacked(contractName))) {
  ...
  function pauseAll() external onlyOwner {
    for (uint256 i = 0; i < controlledContractNames.length; ++i) {
      address addr =
        IContractAddressManager(contractAddressManager).getContractAddressForName(controlledContractNames[i]);
      _pauseContract(addr);
    }
  }
  ...
  function pauseContract(string calldata contractName) external onlyOwner {
    address addr = IContractAddressManager(contractAddressManager).getContractAddressForName(contractName);
    _pauseContract(addr);
  }
