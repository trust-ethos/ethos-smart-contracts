...
abstract contract SignatureControl is Initializable {
  ...
  function __signatureControl_init(address expectedSignerAddr, address signatureVerifierAddr)
    internal
    onlyInitializing
  {
   ...
   */
  function validateAndSaveSignature(bytes32 messageHash, bytes calldata signature)
    internal
    onlyUnusedSignature(signature)
  {
    bool isValid = ISignatureVerifier(signatureVerifier).verifySignature(expectedSigner, messageHash, signature);
   ...
   * @param signatureVerifierAddr SignatureVerifier contract address.
   */
  function _updateSignatureVerifier(address signatureVerifierAddr) internal onlyNonZeroAddress(signatureVerifierAddr) {
